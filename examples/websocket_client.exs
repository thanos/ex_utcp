...
# This example demonstrates how to use the UTCP client with WebSocket providers.
...
defmodule WebSocketClientExample do
  def run do
    IO.puts("=== UTCP WebSocket Client Example ===")
    ...
    provider = Providers.new_websocket_provider([
      ...
      keep_alive: true,
      headers: %{
        "User-Agent" => "ExUtcp-WebSocket/0.1.0"
      }
    ])

    # Register the provider
    IO.puts("\n=== Registering WebSocket Provider ===")
    case Client.register_tool_provider(client, provider) do
      {:ok, tools} ->
        IO.puts("Successfully registered WebSocket provider with #{length(tools)} tools:")
        ...
        case Client.search_tools(client, "", 10) do
          {:ok, found_tools} ->
            ...
            if length(found_tools) > 0 do
              ...
              IO.puts("\n=== Tool Call Test ===")
              IO.puts("Calling tool '#{tool.name}' with args: %{message: \"Hello WebSocket!\"}")

              case Client.call_tool(client, tool.name, %{"message" => "Hello WebSocket!"}) do
                {:ok, result} ->
      ...
      {:error, reason} ->
        ...
        IO.puts("Note: This example requires a WebSocket server running on ws://localhost:8080/ws")
    end
