defmodule ExUtcp.Monitoring.PerformanceTest do
  ...
  describe "Performance Monitoring" do
    test "measures function execution time" do
      result = Performance.measure("test_operation", %{test: true}, fn ->
        Process.sleep(10)
        "test_result"
      end)
    ...
    test "measures tool call performance" do
      result = Performance.measure_tool_call("test_tool", "test_provider", %{"arg" => "value"}, fn ->
        Process.sleep(5)
        {:ok, %{"result" => "success"}}
      end)
    ...
    test "measures search performance" do
      result = Performance.measure_search("test query", :fuzzy, %{providers: []}, fn ->
        Process.sleep(5)
    ...
    test "measures connection performance" do
      result = Performance.measure_connection("test_provider", :http, fn ->
        Process.sleep(5)
        {:ok, "connected"}
      end)
    ...
    test "gets performance alerts" do
      alerts = Performance.get_performance_alerts(%{}) # Pass empty metrics map
    ...
    test "records custom metrics" do
      ...
      assert :ok = Performance.record_custom_metric("test_histogram", :histogram, 100, %{label: "test"})
      assert :ok = Performance.record_custom_metric("test_summary", :summary, 200, %{label: "test"})
    end
  end
end
