defmodule ExUtcp.OpenApiConverter.Generator do
  ...
  defp build_tool_description(operation) do
    ...
    description_parts = if operation.summary do
      [operation.summary | description_parts]
    else
      description_parts
    end

    description_parts = if operation.description do
      [operation.description | description_parts]
    else
      description_parts
    end

    description_parts = if operation.deprecated do
  ...
  defp add_path_parameters(operation, properties, required) do
    ...
    {new_properties, new_required} = Enum.reduce(path_params, {properties, required}, fn param, {props, req} ->
  ...
  defp add_query_parameters(operation, properties, required) do
    ...
    {new_properties, new_required} = Enum.reduce(query_params, {properties, required}, fn param, {props, req} ->
      param_schema = build_parameter_schema(param)
      new_props = Map.put(props, param.name, param_schema)
      new_req = if param.required, do: [param.name | req], else: req
      {new_props, new_req}
  ...
  defp add_header_parameters(operation, properties, required) do
    ...
    {new_properties, new_required} = Enum.reduce(header_params, {properties, required}, fn param, {props, req} ->
      param_schema = build_parameter_schema(param)
      new_props = Map.put(props, param.name, param_schema)
      new_req = if param.required, do: [param.name | req], else: req
      {new_props, new_req}
  ...
  defp build_parameter_schema(param) do
    base_schema = case param.schema do
      nil -> %{"type" => "string"}
  ...
  defp convert_schema_to_json_schema(schema) do
    ...
    base_schema = if schema.format do
      Map.put(base_schema, "format", schema.format)
    else
      base_schema
    end

    base_schema = if schema.enum do
      Map.put(base_schema, "enum", schema.enum)
    else
      base_schema
    end

    base_schema = if schema.properties && map_size(schema.properties) > 0 do
      converted_props = Enum.map(schema.properties, fn {name, prop_schema} ->
    ...
    base_schema = if schema.items do
  ...
  defp build_output_schema(operation) do
    case operation.responses do
      ...
      responses ->
        ...
        success_response = Enum.find(responses, fn response ->
  ...
  defp build_auth_from_security(operation, parsed_spec) do
    case operation.security do
      ...
      [security_requirement | _] ->
        AuthMapper.map_security_requirement(security_requirement, parsed_spec.components.security_schemes)
      _ -> nil
    end
  end
